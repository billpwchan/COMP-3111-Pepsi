...
public class DataFilterManager {
	...
	public static DataTable NumberFilterSet(final String columnNamechosen, final String Operatorusing, final Float f,
			final DataTable sampleDataTable) {
	...
	public static DataTable TextFilterSet(final String textChosenInTextField, final List<String> checkeditems,
			final DataTable sampleDataTable) {
		...
		for (int i = 1; i < sampleDataTable.getNumRow(); i++) {
			...
			for (int j = 0; j < checkeditems.size(); j++) {
				//
				// System.out.println(
				// "The number of row is " + sampleDataColumn.getData()[i] + "==" +
				// checkeditems.get(j));
...
// fileChooser.LoadFileChooser(stage, "Please select .csv dataset for import",
...
// * @throws IOException
// */
// public static void dataExport(DataTable dataTable, Stage stage) throws
// IOException {
// if (dataTable == null) {
// return;
// }
...
// chooser.saveFileChooser(stage, "Save .csv file", "Comma Delimited File
// (*.CSV)", "*.csv", "user.home",
// "DataSet.csv");
...
// return "DataManager [getClass()=" + getClass() + ", hashCode()=" + hashCode()
// + ", toString()="
// + super.toString() + "]";
// }
